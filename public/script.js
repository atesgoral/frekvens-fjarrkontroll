...
const GUTTER = 2;
const CUBE_WIDTH =
  GUTTER * 2 + PIXEL_RADIUS * 2 * COLS + PIXEL_SPACING * (COLS - 1);
const CUBE_HEIGHT =
  GUTTER * 2 + PIXEL_RADIUS * 2 * ROWS + PIXEL_SPACING * (ROWS - 1);
...
for (let dy = 0; dy < DIFFUSE_DIAMETER; dy++) {
  for (let dx = 0; dx < DIFFUSE_DIAMETER; dx++) {
    ...
    const falloff =
      d > DIFFUSE_DISTANCE ? 1 : (d / (DIFFUSE_DISTANCE + 1)) ** 0.05;
...
yellowEl.addEventListener('mousedown', () => client.emit('yellowDown'));
yellowEl.addEventListener('mouseup', () => client.emit('yellowUp'));
redEl.addEventListener('mousedown', () => client.emit('redDown'));
redEl.addEventListener('mouseup', () => client.emit('redUp'));
...
frontCtx.scale(frontEl.width / CUBE_WIDTH, frontEl.height / CUBE_HEIGHT);

maskCtx.scale(maskEl.width / CUBE_WIDTH, maskEl.height / CUBE_HEIGHT);
      ...
      for (let dy = 0; dy < DIFFUSE_DIAMETER; dy++) {
        ...
        for (let dx = 0; dx < DIFFUSE_DIAMETER; dx++) {
          ...
          levels[y * COLS + x] = Math.min(
            ...
            levels[y * COLS + x] +
              pixels[row * COLS + col] *
                diffuseFilter[dy * DIFFUSE_DIAMETER + dx],
          );
        }
      }
    }
...
function watchMidiInputs(access) {
  for (let input of access.inputs.values()) {
    input.onmidimessage = (message) => {
      ...
      client.emit('midi', {status, data1, data2});
    };
  }
}
